...
int main(int argc,char** argv){
	...
	printf("********************************************************\n");
	for(int32_t i=-2*(1000L*1000L*1000L);i<2*(1000L*1000L*1000L);i+=200000){
		OutputStream out=openOutputStream("test.uleb");
		writeLeb(out,i);
		closeOutputStream(out);
		InputStream in=open("test.uleb");
	...
	printf("********************************************************\n");
	for(int64_t i=-12L*(1000*1000*1000);i<12L*(1000*1000*1000);i+=1000000){
		OutputStream out=openOutputStream("test.uleb");
		writeLeb64(out,i);
		closeOutputStream(out);
		InputStream in=open("test.uleb");
